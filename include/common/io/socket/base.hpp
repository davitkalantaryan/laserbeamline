...
#define __common_io_socket_base_hpp__
...
#ifdef	_WIN32
typedef SOCKET	socketNativeType;
#else
typedef SOCKET	socketNativeType;
#endif
...
public:
	...
	void SetNewSocketDescriptor(socketNativeType rawSocket);
	void ResetSocketWithoutClose();
	socketNativeType GetAndResetSocket();
...
protected:
	socketNativeType   m_handle;
};


#ifdef _SELECT_NEEDED_
struct STypeAndIndex { int index; char whichd; char reserver[3]; };
template <typename TypeCntRd, typename TypeCntWr = TypeCntRd, typename TypeCntEr = TypeCntWr,typename TypeCntRet = std::vector<STypeAndIndex>>
int	selectMltCpp3(
...
#endif  // #ifndef __common_io_socket_base_hpp__
